...
class MessagingExperience {
	...
	func initialize() -> Bool {
		...
		guard let applicationID = AppSettings.shared.applicationID
			, let providerID = AppSettings.shared.providerID
			, let baseDomain = AppSettings.shared.baseURL
			else { return false }
...
extension MessagingNotificationManager: MessagingNotificationDelegate {
	...
	func didReceiveMessagingNotification(_ notification: MessagingNotification) {
		if let listing = ListingManager.shared.listings().filter({ $0.referenceID == notification.referenceID }).first {
			// give time for listingsViewController to load when launching app
			delay(0.1, closure: {
